...
- Chrome の `beforeinstallprompt` に相当する自動インストールバナーは存在しない。ユーザーは
  ...
  → オンボーディング（#24）でこの手動手順を案内する画面を追加した。詳細・検証方法は
  `docs/design-decisions.md` の「オンボーディング（iOS のホーム画面追加案内を含む） (#24)」節を参照
...
- iOS のオンボーディング案内文（#24）: 対応済み（`docs/design-decisions.md` 参照）
