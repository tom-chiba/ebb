...
unknown>)` という非ジェネリックな型のため、失敗時の返り値は `action` がリテラル型に
  ...
  - **プレビューが `MAX_BATCH_STATEMENTS` の上限チェックを一切通らず、確定操作だけが
    ...
    悲観的に見積もった対象メモ数の上限（`exceedsBatchStatementLimit`）をプレビュー
    側にも追加し、確定が拒否しうるケースを常にプレビュー時点で検知するようにした
    （悲観的見積もりのため、実際には統計上限内に収まるはずのケースをプレビュー側が
    過剰に拒否することはあり得るが、安全側であるため許容する）。
