...
> **#77 による訂正（Android 実機で確認）**: 本ドキュメントの「Chrome / FCM は
> legacy `aesgcm` を受理・復号・表示できた」という実測結果は、**デスクトップ
> Chrome のみ**での確認であり、Android 実機では通知が表示されなかった
> （FCM は受理していたが端末側で復号・表示されず）。`packages/push` は
> RFC 8291 の `aes128gcm` 自前実装（`crypto.subtle` ベース）に置き換え済み。
> 以下の本文はデスクトップ検証時点の記録として残す。
...
> **#77 による訂正**: 上記はデスクトップ Chrome のみの実測結果だった。
> Android 実機（#9 で保留していた確認）では FCM が受理（201）しても端末に
> 通知が表示されず、legacy `aesgcm` は Android では機能しないことが判明した。
> #77 で `packages/push` を RFC 8291 の `aes128gcm` 自前実装に置き換えた。
